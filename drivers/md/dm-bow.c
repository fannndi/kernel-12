...
{
	return (range_top(br) - br->sector) * DM_BOW_SECTOR_SIZE;
}

static sector_t bvec_top(struct bvec_iter *bi_iter)
{
	return bi_iter->bi_sector + bi_iter->bi_size / DM_BOW_SECTOR_SIZE;
}
		...
		char **argv)
{
	...
	while (opt_params--) {
		...
		if (sscanf(opt_string, "block_size:%u%c",
			...
			if (bc->block_size < DM_BOW_SECTOR_SIZE ||
			    bc->block_size > 4096 ||
...
static void bow_write(struct work_struct *work)
{
	...
	do {
		...
		bi_iter.bi_sector += bi_iter.bi_size / DM_BOW_SECTOR_SIZE;
